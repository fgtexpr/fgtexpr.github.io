    ...
    def __init__(self, dxdt, dt, n_nodes):
        ...
        for i, n in enumerate(self.nodes):
            for j, m in enumerate(self.nodes):
                ...
                if ((m != n and dist <= 2) or i == 0) and j != 0:
                    kick_fnc = lambda x: (math.cos(x) + 2 )*0.02/(dist + 1) if i != 0 else  (math.cos(x + 3.14) + 2)
                    n.add_neighbor(m, kick_fnc)
        m = math.ceil(math.sqrt(n_nodes))
        for n in self.nodes:
            n.position = (0.5/m + n.position[0]/m, 0.5/m + n.position[1]/m)
